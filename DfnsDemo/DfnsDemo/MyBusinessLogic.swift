...
final class MyBusinessLogic: ObservableObject {
    ...
    public func signMessage(message: String, walletId: String, authToken: String, passkeysSigner: PasskeysSigner) async -> String {
        ...
        let result = await myServer.completeSignature(walletId: walletId, appId: self.appId, authToken: authToken, requestBody: initWalletResult.response.requestBody, signedChallenge: userActionAssertion)
